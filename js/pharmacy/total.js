// total.js V6 FIXED - شيلنا تصدير الاكسل منه
...
@media(max-width:900px){.pro-grid{grid-template-columns:1fr}}
    ...
    <div class="card" style="margin-top:12px"><div class="card-h"><span>📋 تفاصيل اليومية</span><div class="cat-pills" id="catPills"></div></div><div id="totalTableCard" style="max-height:45vh;overflow:auto"></div><div id="totalPager" style="display:flex;gap:5px;justify-content:center;padding:8px"></div></div>
...
function renderTable() {
  ...
  document.getElementById('proStats').innerHTML = `<div style="background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:12px"><div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px"><b>📊 ملخص احترافي بالنسب - ${selectedMonthKey === 'all'? 'الكل' : getMonthNameOnly(selectedMonthKey)}</b><div style="display:flex;gap:6px;align-items:center"><span style="font-size:11px;font-weight:800">نسبة ربحي</span><input id="myP" class="mini-input" type="number" value="${saved.my}" oninput="updatePro()"><span>%</span></div></div><div class="kpi-row"><div class="kpi"><div><div style="font-size:10px;color:#64748b">إجمالي البيع</div><b>${totalSales.toLocaleString()}</b></div><span>💰</span></div><div class="kpi"><div><div style="font-size:10px;color:#64748b">المصروف الفعلي</div><b>${totalExpenses.toLocaleString()}</b></div><span>📦</span></div><div class="kpi"><div><div style="font-size:10px;color:#64748b">الصافي قبل ربحك</div><b>${(totalSales - totalExpenses).toLocaleString()}</b></div><span>📈</span></div><div class="kpi" style="background:#f0fdf4"><div><div style="font-size:10px">ربحي</div><b id="myV">0</b></div><span>👤</span></div><div class="kpi" style="background:#fff7ed"><div><div style="font-size:10px">الباقي للتوزيع</div><b id="remV">0</b></div><span>🧮</span></div></div><div class="pro-grid">${window.FINAL_CATS_TASNEF.map(cat => { let actual = expensesByCat[cat] || 0; let perc = totalExpenses? (actual / totalExpenses * 100).toFixed(1) : 0; let col = cat === 'دواء'? '#0f766e' : cat === 'كوزمتكس'? '#7c3aed' : '#dc2626'; let allocPerc = saved.dist[cat] || 0; return `<div style="border:1px solid #f1f5f9;border-radius:14px;padding:10px;border-top:3px solid ${col}"><div style="display:flex;justify-content:space-between"><b><span style="display:inline-block;width:8px;height:8px;background:${col};border-radius:50%"></span> ${cat}</b><small>${perc}%</small></div><div style="font-size:20px;font-weight:900;margin-top:4px">${actual.toLocaleString()}</div><div class="bar-bg" style="margin-top:6px"><div class="bar-f" style="width:${perc}%;background:${col}"></div></div><div style="display:flex;gap:6px;align-items:center;margin-top:8px"><span style="font-size:11px">مخصص:</span><input data-cat="${cat}" class="mini-input percI" type="number" value="${allocPerc}" oninput="updatePro()"><span style="font-size:10px">%</span><b data-res="${cat}" style="margin-right:auto;background:#f8fafc;padding:3px 8px;border-radius:6px">0</b></div></div>`; }).join('')}</div><div id="sumErr" style="text-align:center;margin-top:8px;font-size:11px;font-weight:800"></div></div>`;
  updatePro(); document.getElementById('catPills').innerHTML = ['الكل',...window.FINAL_CATS_TASNEF].map(c => `<button class="cat-pill ${_activeCatFilter === c? 'active' : ''}" onclick="setCatFilter('${c}')">${c}</button>`).join(''); filterSuppliers(); renderTrend(); renderDailyRows();
}
function updatePro() {
  ...
  document.querySelectorAll('[data-res]').forEach(el => { let cat = el.dataset.res; let p = dist[cat] || 0; let alloc = rem * p / 100; el.textContent = alloc.toLocaleString(); });
...
function renderTrend() {
  ...
  document.getElementById('dailyTrend').innerHTML = keys.map(k => { let r = trend[k]; return `<tr><td>${k}</td><td>${(r['دواء'] || 0).toLocaleString()}</td><td>${(r['كوزمتكس'] || 0).toLocaleString()}</td><td>${(r['مصاريف'] || 0).toLocaleString()}</td><td style="font-weight:900">${(r.total || 0).toLocaleString()}</td></tr>`; }).join('') || '<tr><td colspan=5>فاضي</td></tr>';
}
...
window.renderTotal = renderTotal; window.filterSuppliers = filterSuppliers; window.setCatFilter = setCatFilter; window.renderDailyRows = renderDailyRows; window.updatePro = updatePro;
